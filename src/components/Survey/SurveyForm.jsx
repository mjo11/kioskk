...
const questionss = [
  {
    message: "success retrieving questionaires",
    data: [
      {
        id: 1,
...
const fetchQuestion = async () => {
  try {
    const response = await fetch("http://localhost:3000/survey/user/questions");
...
const SurveyForm = () => {
  ...
  useEffect(() => {
    const getQuestions = async () => {
      const data = questionss[0].data; // await fetchQuestion()
      setQuestion(data); //pass the data to use state
    };
    getQuestions();
  }, []);
